...
| Offline downloads | Resumable desktop transfers and standalone `.mka` files | Native background HLS packages, durable Rust job state, offline playback, deletion, and one-action FLAC/M4A export implemented; physical-device lifecycle and exported metadata validation remain | P1 |
