...
Use GitHub Actions with a hosted `macos-latest` runner for unsigned simulator
builds and compile checks. Once signing credentials and provisioning are
available, the same workflow can archive a signed IPA. Keep certificates,
...
and initial configuration through Xcode.
...
- [Bitrise iOS deployment](https://devcenter.bitrise.io/en/deploying/ios-deployment.html)
